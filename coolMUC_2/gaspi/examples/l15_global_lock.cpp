...
gaspi_return_t global_lock_init(const gaspi_segment_id_t	seg,
...
try_lock_t global_try_lock(const gaspi_segment_id_t	seg,
			const gaspi_offset_t		off,
			const gaspi_rank_t		rank_loc)
{
	gaspi_rank_t iProc;
	...
	SUCCESS_OR_DIE(gaspi_atomic_compare_swap(seg, off, rank_loc, VAL_UNLOCKED,
							iProc, &old_value, GASPI_BLOCK));

	return (old_value == VALUE_UNLOCKED) ? LOCK_ACQUIRED : NO_LOCK_ACQUIRED;
}

void global_unlock(const gaspi_segment_id 	seg,
		const gaspi_offset_t		off,
		const gaspi_rank_t		rank_loc)
{
	gaspi_rank_t iProc;
	...
	gaspi_atomic_value_t old_value;
	SUCCESS_OR_DIE(gaspi_atomic_compare_swap(seg, off, rank_loc, iProc,
						VAL_UNLOCKED, &old_value, GASPI_BLOCK));
